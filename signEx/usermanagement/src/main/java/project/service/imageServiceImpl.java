...
public class imageServiceImpl implements ImageServiceInter{
    ...
    @Override
    public ResponseEntity<ImageEntity> getImage(int idUser) {
        Optional<ImageEntity> retrivedImage = imageRepository.findByUserEntityId(idUser);
        if(retrivedImage.isPresent())
        {
            ImageEntity img =retrivedImage.get();
    ...
    public ResponseEntity<String> updateImage(MultipartFile file, int idUser) throws IOException {
        ...
        if(userOptional.isPresent())
        {
            ...
            ImageEntity image=user.getUserImage();
